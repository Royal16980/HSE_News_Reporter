 ...
 *
 * Full NOVA-PRIME (Opus 4.7 stateful Ruflo orchestrator) is built across Phase 1–2.
 * This hello-world validates:
 *   - Supabase write to editorial_decisions + agent_runs
...
- Last editorial decision: ${state.lastEditorialDecision ?? 'none (first cycle)'}

Respond with JSON:
{
  "decision_type": "agenda_priority | swarm_directive | quality_check | escalation",
  "rationale": "one sentence — the decision and its reason",
  "priority_action": "what the next swarm cycle should focus on",
  "directives": { "swarm": "research|editorial|governance|ops", "instruction": "terse instruction string" }
}`
  ...
  try {
    ...
    parsed = {
      ...
      rationale: 'Hello-world cycle — NOVA-PRIME initialised and operational on safetynews.pro.',
      directives: { swarm: 'research', instruction: 'Begin content aggregation from HSE.gov.uk.' },
    }
  }
...
async function logDecision(params: {
  ...
  if (error) throw new Error(`editorial_decisions insert failed: ${error.message}`)
}
...
// Approximate Opus 4.7 pricing — update if Anthropic revises
// Input: ~$15/MTok, Output: ~$75/MTok → pence
const PENCE_PER_INPUT_TOKEN = 1500 / 1_000_000
const PENCE_PER_OUTPUT_TOKEN = 7500 / 1_000_000
...
export async function runNovaPrimeCycle(): Promise<{
  ...
  try {
    ...
    console.log(`[NOVA-PRIME] ${durationMs}ms · ${decision.rationale}`)

    return { success: true, decisionType: decision.decisionType, rationale: decision.rationale, costPence, durationMs }
  } catch (err) {
    const durationMs = Date.now() - startMs
